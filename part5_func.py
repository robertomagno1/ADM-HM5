...
def plot_girvan_newman_communities(graph, communities, removed_edges):
    """
    Visualize the graph with Girvan-Newman detected communities and highlight removed edges.
    
    Args:
        graph (Graph): The graph object (unweighted, undirected).
        communities (list): List of communities detected.
        removed_edges (list): List of removed edges.
    """
    ...
    # Define node positions using spring layout for better visibility
    pos = nx.spring_layout(G, seed=42)
    
    plt.figure(figsize=(14, 10))
    
    # Draw edges excluding removed edges
    edges_to_draw = [(u, v) for u, v in G.edges() if (u, v) not in removed_edges and (v, u) not in removed_edges]
    nx.draw_networkx_edges(G, pos, edgelist=edges_to_draw, edge_color='gray', alpha=0.5)
    
    # Highlight removed edges in red
    nx.draw_networkx_edges(G, pos, edgelist=removed_edges, edge_color='red', width=2, label='Removed Edges')
    ...
    # Add labels to nodes (optional, can be removed for larger graphs)
    nx.draw_networkx_labels(G, pos, font_size=8, font_color='black')
    
    # Add legend and title
    plt.legend(scatterpoints=1)
    plt.title("Girvan-Newman Community Detection with Removed Edges", fontsize=14)
    plt.axis('off')
    plt.show()


def calculate_modularity_airport(graph, communities):
    """
    Calculate the modularity airport graph partitioned into communities.
    """
    # Get adjacency matrix and node order
    adj_matrix, nodes = graph.adjacency_matrix()
    node_idx = {node: i for i, node in enumerate(nodes)}
    ...
    for community in communities:
        for i in range(len(community)):
            for j in range(len(community)):
                node_i = node_idx[community[i]]
                node_j = node_idx[community[j]]
                A_ij = adj_matrix[node_i, node_j]  # Adjacency matrix value
                k_i = np.sum(adj_matrix[node_i])  # Degree of node i
                k_j = np.sum(adj_matrix[node_j])  # Degree of node j
